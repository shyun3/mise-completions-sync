...
def main():
    ...
    print()
    print("### Tools whose binary has a different name")
    print()
    print("A completion file is only loaded when it is named after the command the")
    print("user types, so an entry keyed on a mise tool name that differs from its")
    print("binary has to say so with `completion_name`. `television` installs `tv`:")
    print()
    print("```toml")
    print(
        'television = { completion_name = "tv", zsh = "tv init zsh", '
        'bash = "tv init bash", fish = "tv init fish" }'
    )
    print("```")
    print()
